...
var buildWebpack = require('./webpack.js');
var os = require('os');
require('colors');
...
Actions.prototype.deploy = function() {
  ...
  var buildLocation = path.resolve(os.tmpdir());
  ...
  var packWithWebpack = this.config.webpack;

  async.waterfall([
    function (next) {
      if (packWithWebpack) {
        console.log('Building Webpack: ' + appPath);
        buildWebpack(appPath, next);
      } else {
        next(null);
      }
    },
    function () {
      console.log('Building Started: ' + appPath);
      buildApp(appPath, meteorBinary, buildLocation, _.last(arguments));
    },
  ], function (err) {
    if (err) {
      process.exit(1);
